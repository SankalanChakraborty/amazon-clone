...
const SignUp = () => {
  ...
  const checkformValidation = (e) => {
    ...
    if (
      ...
      newUserData.setPassword !== newUserData.confirmPassword
    ) {
      setError({
        passwordShort: false,
        ...
        <form action="/signup" method="POST" onSubmit={registerUser}>
          ...
          <div className="set-password__section">
            ...
            <span
              className="tooltip"
              style={
                error.passwordShort ? { display: "block" } : { display: "none" }
              }
            >
              <i className="fa-solid fa-lightbulb"></i>Password must contain at
              least six characters
            </span>
            <input
              className={error.passwordShort ? "error-class" : ""}
              id="set-password"
              name="setPassword"
              type="password"
              placeholder="At least 6 characters"
              value={newUserData.setPassword}
              onChange={storeNewUserData}
              onBlur={checkformValidation}
              required
            />
          </div>
          <div className="confirm-password__section">
            ...
            >
              <i className="fa-solid fa-lightbulb"></i>Passwords do not match
            </span>
            <input
              className={error.passwordDontMatch ? "error-class" : ""}
              id="confirm-password"
